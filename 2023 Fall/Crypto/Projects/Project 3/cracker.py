...
def main():
	...
	if args.key:
		...
		crack(ciphertexts, cleartexts, target, args.getkey)
...
def crack(ciphertexts: List[bytes], cleartexts: List[bytearray], target, getkey: bool) -> None:
	...
	for column in range(max_length):  # go over characters from the beginning of lines
		pending_ciphers = [line for line in ciphertexts if len(line) > column] # copying ciphertexts to pending ciphers
		
		i = 0
		for cipher in pending_ciphers: # for each cipher in pending ciphers - which is just a copy of ciphertexts
